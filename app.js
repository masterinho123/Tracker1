...
const Header = ({ totalHabits, completedHabits, totalPossible, currentDate, onNext, onPrev, onNextYear, onPrevYear, onExport, onImportClick, syncCode, setSyncCode, syncStatus, syncError, apiBase, setApiBase, supabaseConfig, setSupabaseConfig }) => {
...
const SchoolGradesView = ({ schoolData, setSchoolData }) => {
    ...
    const [newMaxGrade, setNewMaxGrade] = useState(10);
    ...
    const addSubject = (e) => {
        ...
        const subject = {
            ...
            maxGrade: parseFloat(newMaxGrade) || 10,
            grades: []
        };
        setSchoolData(prev => ({
            ...prev,
    ...
    const addGrade = (subjectId, value, date = new Date().toISOString().split('T')[0]) => {
        const subject = schoolData.subjects.find(s => s.id === subjectId);
        const max = subject?.maxGrade || 10;
    ...
    const totalAveragePercent = useMemo(() => {
        const percents = schoolData.subjects
            .map(s => {
                ...
                return avg > 0 ? (avg / (s.maxGrade || 10)) : null;
            })
            .filter(p => p !== null);
    ...
    const stats = useMemo(() => {
        const subjectsWithRelativeAvg = schoolData.subjects
            .map(s => {
                const avg = parseFloat(calculateAverage(s.grades));
                return { ...s, avg, relative: avg / (s.maxGrade || 10) };
            })
            .filter(s => s.avg > 0);
    ...
    const totalAverageDisplay = (totalAveragePercent * 10).toFixed(2);
                ...
                <div className="bg-[#0f172a] border border-[#1f2937] rounded-3xl p-6 shadow-xl relative overflow-hidden group">
                    ...
                    <div className="relative">
                        ...
                        <div className="mt-4 flex items-center gap-2 text-sm text-slate-400">
                            ...
                            Media: {stats.best ? `${stats.best.avg} / ${stats.best.maxGrade || 10}` : '0.00'}
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-[#0f172a] border border-[#1f2937] rounded-3xl p-6 shadow-xl relative overflow-hidden group">
                    ...
                    <div className="relative">
                        ...
                        <div className="mt-4 flex items-center gap-2 text-sm text-slate-400">
                            ...
                            Media: {stats.worst ? `${stats.worst.avg} / ${stats.worst.maxGrade || 10}` : '0.00'}
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            return (
                                <div key={subject.id} className="space-y-2">
                                    ...
                                    <div className="h-1.5 w-full bg-[#1e293b] rounded-full overflow-hidden">
                                        <div 
                                            className={`h-full transition-all duration-500 bg-gradient-to-r ${getGradeBarColor(avg)}`}
                ...
                <div className="flex gap-4">
                    <div className="relative flex-1 md:w-32">
                        <input 
                            ...
                            placeholder="Max"
                            className="w-full bg-[#0f172a] border border-[#1f2937] rounded-2xl px-4 py-4 text-slate-200 focus:outline-none focus:border-blue-500 transition-all text-center font-bold"
                        />
                        <div className="absolute -top-2 left-4 px-2 bg-[#020617] text-[10px] font-bold text-slate-500 uppercase tracking-widest">Max Voto</div>
                    </div>
                    <button 
                        type="submit"
                            ...
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex-1">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex items-center gap-1 bg-slate-800/50 px-2 py-0.5 rounded-lg border border-slate-700/50">
                                            <span className="text-[10px] font-bold text-slate-500 uppercase">Max</span>
                                            <input 
                                                type="number"
                                                value={max}
                                                onChange={(e) => updateMaxGrade(subject.id, e.target.value)}
                                                className="w-6 bg-transparent text-xs font-black text-slate-300 focus:outline-none focus:text-blue-400"
                                            />
                                        </div>
                                    </div>
                            ...
                            <div className="flex flex-wrap gap-3 mb-6 min-h-[48px]">
                                {subject.grades.map(grade => (
                                    <div key={grade.id} className="relative group/grade">
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-sm border-2 transition-all hover:scale-110 shadow-lg ${getGradeColor(grade.value, max)}`}>
                                            {grade.value}
                                        </div>
                                        <button 
                                ...
                                {addingGradeTo === subject.id ? (
                                    <div className="flex items-center gap-2 bg-[#1e293b] p-1 rounded-2xl border border-blue-500/50 shadow-lg shadow-blue-900/20 animate-in zoom-in-95 duration-200">
                                        <input 
                                            autoFocus
                                            type="number" step="0.5" min="0" max={max} placeholder="Voto"
                                            value={tempGrade}
                                            onChange={(e) => setTempGrade(e.target.value)}
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    addGrade(subject.id, tempGrade);
                                        ...
                                        <button 
                                            ...
                                            className="bg-blue-600 p-2 rounded-xl text-white hover:bg-blue-500 transition-all"
                                        >
                                    ...
                                    <button 
                                        onClick={() => setAddingGradeTo(subject.id)}
                            ...
                            return (
                                <div key={dateKey} className={`h-24 md:h-32 border-b border-r border-[#1f2937] p-2 relative transition-colors hover:bg-[#1e293b]/20 ${isToday ? 'bg-blue-600/5' : ''}`}>
                                    ...
                                    <div className="mt-1 space-y-1 overflow-y-auto max-h-[calc(100%-1.5rem)]">
                                        ...
                                        {dayGrades.map(g => (
                                            <div key={g.id} className={`text-[9px] font-bold ${getGradeColor(g.value).split(' ')[1]} ${getGradeColor(g.value).split(' ')[0]} p-1 rounded-md truncate shadow-sm`}>
...
const App = () => {
    ...
    const [schoolData, setSchoolData] = useState(() => {
        ...
        const defaultData = { subjects: [], tests: [] };
        try { 
    ...
    return (
        <div className="flex min-h-screen bg-[#0b1220] text-slate-300 font-sans selection:bg-blue-500/30">
            {/* Sidebar Navigation */}
            <div className="w-16 md:w-20 bg-[#0f172a] border-r border-[#1f2937] flex flex-col items-center py-8 gap-8 flex-shrink-0">
                ...
                <button 
                    ...
                    className={`p-3 rounded-xl transition-all duration-200 ${activeTab === 'tracker' ? 'bg-blue-600/20 text-blue-400 shadow-inner' : 'text-slate-500 hover:text-slate-300 hover:bg-[#1e293b]'}`}
                    title="Tracker"
                >
                ...
                <button 
                    ...
                    className={`p-3 rounded-xl transition-all duration-200 ${activeTab === 'new_view' ? 'bg-blue-600/20 text-blue-400 shadow-inner' : 'text-slate-500 hover:text-slate-300 hover:bg-[#1e293b]'}`}
                    title="Nuova Vista"
                >
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
