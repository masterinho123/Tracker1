...
const SchoolGradesView = ({ schoolData, setSchoolData, isLight }) => {
    ...
    const getGradeColor = (value, max = 10, min = 0, isLight = false) => {
        ...
        if (rel < 0.25) return isLight ? 'text-red-600 bg-red-100 border-red-200' : 'text-red-500 bg-red-500/10 border-red-500/20 shadow-red-900/10';
    ...
    const getGradeBarColor = (value, max = 10, min = 0, isLight = false) => {
        ...
        if (rel < 0.25) return isLight ? 'from-red-600 to-red-400' : 'from-red-600 to-red-400';
        ...
        <div className="max-w-5xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <div className="flex flex-col items-end gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isLight ? 'text-slate-400' : 'text-slate-500'} mr-4`}>Scala Voti</span>
                    <div className={`flex items-center gap-3 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border p-2.5 rounded-[2rem] shadow-xl transition-all duration-300 hover:shadow-2xl`}>
                        <div className={`flex items-center justify-center w-10 h-10 rounded-2xl ${isLight ? 'bg-slate-50 text-slate-400' : 'bg-[#020617] text-slate-500'}`}>
                        ...
                        <div className="flex gap-3 px-1">
                            <div className="flex flex-col">
                                <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.15em] mb-1">Min</span>
                                <div className={`flex items-center h-8 px-3 rounded-xl border ${isLight ? 'bg-slate-50 border-slate-200 focus-within:border-orange-400' : 'bg-[#020617] border-[#1f2937] focus-within:border-orange-500'} transition-all duration-200`}>
                                    <input 
                                        type="number" 
                                        value={schoolData.minGrade || 0}
                                        onChange={(e) => updateGlobalMinGrade(e.target.value)}
                                        className={`w-6 bg-transparent text-sm font-black ${isLight ? 'text-orange-600' : 'text-orange-400'} text-center focus:outline-none`}
                                    />
                                </div>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.15em] mb-1">Max</span>
                                <div className={`flex items-center h-8 px-3 rounded-xl border ${isLight ? 'bg-slate-50 border-slate-200 focus-within:border-blue-400' : 'bg-[#020617] border-[#1f2937] focus-within:border-blue-500'} transition-all duration-200`}>
                                    <input 
                                        type="number" 
                                        value={schoolData.maxGrade || 10}
                                        onChange={(e) => updateGlobalMaxGrade(e.target.value)}
                                        className={`w-6 bg-transparent text-sm font-black ${isLight ? 'text-blue-600' : 'text-blue-400'} text-center focus:outline-none`}
                                    />
                                </div>
                            </div>
                        </div>
        ...
        <div className={`flex min-h-screen ${isLight ? 'bg-slate-50 text-slate-900' : 'bg-[#0b1220] text-slate-300'} font-sans selection:bg-blue-500/30 transition-colors duration-300`}>
            ...
            <div className={`w-16 md:w-20 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border-r flex flex-col items-center py-8 gap-8 flex-shrink-0 transition-colors duration-300`}>
                ...
                <div className="mt-auto flex flex-col gap-4">
                    <button 
                        onClick={changeApiBase}
                        className={`p-3 rounded-xl transition-all duration-200 ${isLight ? 'text-blue-500 hover:bg-blue-50' : 'text-blue-400 hover:bg-blue-400/10'}`}
