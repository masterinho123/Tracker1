...
const SchoolGradesView = ({ 
    ...
    onAddSubject, onDeleteSubject, onAddGrade, onUpdateGrade, onDeleteGrade,
                        ...
                        <Card key={subject.id} isLight={isLight} group className="hover:shadow-xl relative duration-300">
                            ...
                            <div className="flex items-center justify-between mb-4 md:mb-6">
                                <div className="flex-1 min-w-0 mr-4">
                                    ...
                                    <div className="text-[10px] md:text-xs font-bold text-slate-500 mt-0.5 md:mt-1 uppercase tracking-widest">{subject.grades.length} {t.gradesCount}</div>
                                </div>
...
const App = () => {
    ...
    // Header component optimized for navigation
    const Header = ({ title, desc, onNext, onPrev, onNextYear, onPrevYear, isLight, t }) => (
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 md:mb-12">
            <div className="space-y-1">
                <h1 className={`text-4xl md:text-5xl font-black tracking-tight ${isLight ? 'text-slate-800' : 'text-white'} transition-colors duration-300`}>{title}</h1>
                <p className={`${isLight ? 'text-slate-500' : 'text-slate-400'} font-medium text-lg`}>{desc}</p>
            </div>
            <div className="flex items-center gap-2 self-start md:self-center">
                <Card isLight={isLight} noPadding={true} className="flex items-center p-1 shadow-lg">
                    <button onClick={onPrevYear} className={`p-2 rounded-xl ${isLight ? 'hover:bg-slate-100 text-slate-400' : 'hover:bg-slate-800 text-slate-500'} transition-all`} title={t.prevYear}>
                    ...
                    <button onClick={onPrev} className={`p-2 rounded-xl ${isLight ? 'hover:bg-slate-100 text-slate-600' : 'hover:bg-slate-800 text-slate-300'} transition-all`}>
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7" /></svg>
                    </button>
                    <div className={`px-4 py-2 font-black text-sm uppercase tracking-widest ${isLight ? 'text-slate-700' : 'text-slate-200'}`}>
                        {currentDate.toLocaleString(getLocale(language), { month: 'short', year: 'numeric' })}
                    </div>
                    <button onClick={onNext} className={`p-2 rounded-xl ${isLight ? 'hover:bg-slate-100 text-slate-600' : 'hover:bg-slate-800 text-slate-300'} transition-all`}>
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7" /></svg>
                    </button>
                    <button onClick={onNextYear} className={`p-2 rounded-xl ${isLight ? 'hover:bg-slate-100 text-slate-400' : 'hover:bg-slate-800 text-slate-500'} transition-all`} title={t.nextYear}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 5l7 7-7 7M6 5l7 7-7 7" /></svg>
                    </button>
                </Card>
            </div>
        </div>
