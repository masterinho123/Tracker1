...
const SimpleAreaChart = ({ values = [], color = "#10B981", maxY = 100, labels = [], datasets = [] }) => {
    ...
    // Use first dataset for dimensions
    const firstValues = activeDatasets[0].values;
    const w = Math.max(1, firstValues.length - 1);
    const h = 100;
        ...
        >
            <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" width="100%" height="100%">
                    ...
                    {activeDatasets.map((ds, i) => (
                        <React.Fragment key={`defs-${ds.id || i}`}>
                            <linearGradient id={`grad-${ds.id || i}`} x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor={ds.color} stopOpacity="0.18"/>
                                <stop offset="95%" stopColor={ds.color} stopOpacity="0"/>
                            </linearGradient>
                            <filter id={`shadow-${ds.id || i}`} x="-50%" y="-50%" width="200%" height="200%">
                                <feDropShadow dx="0" dy="1" stdDeviation="1" floodColor={ds.color} floodOpacity="0.15"/>
                            </filter>
                        </React.Fragment>
                    ))}
                </defs>
                {activeDatasets.map((ds, i) => {
                    ...
                    const topPath = getSmoothPath(pts, 0.9);
                    const areaPath = `${topPath} L ${w},${h} L 0,${h} Z`;
                    return (
                        <g key={`ds-${ds.id || i}`}>
                            <path d={areaPath} fill={`url(#grad-${ds.id || i})`} />
                            <path d={topPath} fill="none" stroke={ds.color} strokeWidth="1.5" strokeLinecap="round" filter={`url(#shadow-${ds.id || i})`}/>
                        </g>
                    );
                })}
            </svg>
            {hoverIndex !== null && (
                <div style={{ left: `${(hoverIndex/w)*100}%` }} className="absolute top-0 bottom-0 w-px bg-slate-500/30 pointer-events-none">
                            ...
                            {ResponsiveContainer && AreaChart && Area ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData} margin={{ top: 10, right: 6, left: 6, bottom: 0 }}>
                                        ...
                                        <Area type="monotone" dataKey="mood" stroke="#ec4899" strokeWidth={2} fill="url(#colorMood)" />
                                        <Area type="monotone" dataKey="motivation" stroke="#3b82f6" strokeWidth={2} fill="url(#colorMot)" />
                                        {Tooltip && <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '8px', fontSize: '10px' }} />}
                                    </AreaChart>
                                </ResponsiveContainer>
                            ) : (
                                <SimpleAreaChart 
