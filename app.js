...
const SchoolGradesView = ({ schoolData, setSchoolData, isLight }) => {
    ...
    const getGradeColor = (value, max = 10, min = 6, isLight = false) => {
        const val = parseFloat(value);
        const m = parseFloat(max) || 10;
        const n = parseFloat(min) || 6;
        ...
        if (percent < minP - 2) return isLight ? 'text-red-600 bg-red-100 border-red-200' : 'text-red-500 bg-red-500/10 border-red-500/20 shadow-red-900/10';
        if (percent < minP - 1) return isLight ? 'text-orange-700 bg-orange-100 border-orange-200' : 'text-orange-600 bg-orange-600/10 border-orange-600/20 shadow-orange-900/10';
        if (percent < minP) return isLight ? 'text-orange-500 bg-orange-50 border-orange-100' : 'text-orange-400 bg-orange-400/10 border-orange-400/20 shadow-orange-900/10';
        if (percent < minP + 0.75) return isLight ? 'text-amber-600 bg-amber-50 border-amber-100' : 'text-amber-400 bg-amber-400/10 border-amber-400/20 shadow-amber-900/10';
        if (percent < minP + 1.5) return isLight ? 'text-yellow-600 bg-yellow-50 border-yellow-100' : 'text-yellow-500 bg-yellow-500/10 border-yellow-500/20 shadow-yellow-900/10';
        if (percent < minP + 2.25) return isLight ? 'text-lime-600 bg-lime-50 border-lime-100' : 'text-lime-400 bg-lime-400/10 border-lime-400/20 shadow-lime-900/10';
        if (percent < minP + 3.2) return isLight ? 'text-green-600 bg-green-50 border-green-100' : 'text-green-500 bg-green-500/10 border-green-500/20 shadow-green-900/10';
    ...
    const getGradeBarColor = (value, max = 10, min = 6, isLight = false) => {
        const val = parseFloat(value);
        const m = parseFloat(max) || 10;
        const n = parseFloat(min) || 6;
        ...
        if (percent < minP - 2) return isLight ? 'from-red-600 to-red-400' : 'from-red-600 to-red-400';
        if (percent < minP - 1) return isLight ? 'from-orange-700 to-orange-500' : 'from-orange-700 to-orange-500';
        if (percent < minP) return isLight ? 'from-orange-500 to-orange-300' : 'from-orange-500 to-orange-300';
        if (percent < minP + 0.75) return isLight ? 'from-amber-500 to-amber-300' : 'from-amber-500 to-amber-300';
        if (percent < minP + 1.5) return isLight ? 'from-yellow-500 to-yellow-300' : 'from-yellow-500 to-yellow-300';
        if (percent < minP + 2.25) return isLight ? 'from-lime-500 to-lime-300' : 'from-lime-500 to-lime-300';
        if (percent < minP + 3.2) return isLight ? 'from-green-600 to-green-400' : 'from-green-600 to-green-400';
                ...
                <div className={`flex items-center gap-4 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border p-4 rounded-3xl shadow-xl transition-all duration-300`}>
                        <div className="flex flex-col">
                            ...
                            <div className="flex gap-4 mt-1">
                                <div className="flex flex-col">
                                    <span className={`text-[9px] font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase`}>Minimo:</span>
                                    <div className={`flex items-center gap-2 ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#020617] border-[#1f2937]'} px-2 py-1 rounded-xl border transition-all duration-300`}>
                                        <input 
                                            type="number" 
                                            value={schoolData.minGrade || 6}
                                            onChange={(e) => updateGlobalMinGrade(e.target.value)}
                                            className={`w-8 bg-transparent text-sm font-black ${isLight ? 'text-orange-600' : 'text-orange-400'} text-center focus:outline-none`}
                                        />
                                    </div>
                                </div>
                                <div className="flex flex-col">
                                    <span className={`text-[9px] font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase`}>Massimo:</span>
                                    <div className={`flex items-center gap-2 ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#020617] border-[#1f2937]'} px-2 py-1 rounded-xl border transition-all duration-300`}>
                                        <input 
                                            type="number" 
                                            value={schoolData.maxGrade || 10}
                                            onChange={(e) => updateGlobalMaxGrade(e.target.value)}
                                            className={`w-8 bg-transparent text-sm font-black ${isLight ? 'text-blue-600' : 'text-blue-400'} text-center focus:outline-none`}
                                        />
                                    </div>
                                </div>
                            </div>
