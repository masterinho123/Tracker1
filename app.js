        ...
        <div className={`${isLight ? 'bg-white border-slate-200 text-slate-700' : 'bg-[#0f172a] border-[#1f2937] text-slate-200'} border p-5 rounded-xl shadow-sm mb-6 flex justify-between items-center flex-wrap gap-4 transition-colors duration-300`}>
            <div className="flex items-center gap-4 flex-shrink-0">
                ...
                <div className="text-center">
                    <div className={`text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'} uppercase tracking-wide`}>Numero di abitudini</div>
                    <div className={`text-xl font-bold ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>{totalHabits}</div>
                </div>
                <div className="text-center">
                    <div className={`text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'} uppercase tracking-wide`}>Completate</div>
                    <div className={`text-xl font-bold ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>{completedHabits}</div>
                </div>
                <div className="w-48">
                    <div className="flex justify-between mb-1">
                        <span className={`text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'} uppercase tracking-wide`}>Progresso</span>
                        ...
                        return (
                            <div key={habit.id} className={`h-10 border-b ${isLight ? 'border-slate-100' : 'border-[#1f2937]'} flex items-center px-2 transition-colors duration-300`}>
                                ...
                                <div className="flex-grow px-2 transition-colors duration-300" title={`${percentage}%`}>
                                    <ProgressBar value={actual} max={denom} colorClass="bg-green-500" height="h-2" isLight={isLight} />
                                </div>
                            </div>
                        );
                    })}
...
const SchoolGradesView = ({ schoolData, setSchoolData, isLight }) => {
    ...
    const getGradeColor = (value, max = 10, isLight = false) => {
        ...
        if (percent < 4) return isLight ? 'text-red-600 bg-red-100 border-red-200' : 'text-red-500 bg-red-500/10 border-red-500/20 shadow-red-900/10';
        if (percent < 5) return isLight ? 'text-orange-700 bg-orange-100 border-orange-200' : 'text-orange-600 bg-orange-600/10 border-orange-600/20 shadow-orange-900/10';
        if (percent < 6) return isLight ? 'text-orange-500 bg-orange-50 border-orange-100' : 'text-orange-400 bg-orange-400/10 border-orange-400/20 shadow-orange-900/10';
        if (percent < 6.75) return isLight ? 'text-amber-600 bg-amber-50 border-amber-100' : 'text-amber-400 bg-amber-400/10 border-amber-400/20 shadow-amber-900/10';
        if (percent < 7.5) return isLight ? 'text-yellow-600 bg-yellow-50 border-yellow-100' : 'text-yellow-500 bg-yellow-500/10 border-yellow-500/20 shadow-yellow-900/10';
        if (percent < 8.25) return isLight ? 'text-lime-600 bg-lime-50 border-lime-100' : 'text-lime-400 bg-lime-400/10 border-lime-400/20 shadow-lime-900/10';
        if (percent < 9.2) return isLight ? 'text-green-600 bg-green-50 border-green-100' : 'text-green-500 bg-green-500/10 border-green-500/20 shadow-green-900/10';
    ...
    const getGradeBarColor = (value, max = 10, isLight = false) => {
        ...
        if (percent < 4) return isLight ? 'from-red-600 to-red-400' : 'from-red-600 to-red-400';
        if (percent < 5) return isLight ? 'from-orange-700 to-orange-500' : 'from-orange-700 to-orange-500';
        if (percent < 6) return isLight ? 'from-orange-500 to-orange-300' : 'from-orange-500 to-orange-300';
        if (percent < 6.75) return isLight ? 'from-amber-500 to-amber-300' : 'from-amber-500 to-amber-300';
        if (percent < 7.5) return isLight ? 'from-yellow-500 to-yellow-300' : 'from-yellow-500 to-yellow-300';
        if (percent < 8.25) return isLight ? 'from-lime-500 to-lime-300' : 'from-lime-500 to-lime-300';
        if (percent < 9.2) return isLight ? 'from-green-600 to-green-400' : 'from-green-600 to-green-400';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-xs font-bold ${isLight ? 'text-slate-500' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Media Generale</div>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-xs font-bold ${isLight ? 'text-slate-500' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Prossime Verifiche</div>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-xs font-bold ${isLight ? 'text-slate-500' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Migliore</div>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-xs font-bold ${isLight ? 'text-slate-500' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Da Migliorare</div>
                    ...
                    return (
                        <div key={subject.id} className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-sm'} border rounded-3xl p-6 hover:shadow-xl transition-all group relative overflow-hidden duration-300`}>
                            ...
                            <div className={`h-2 w-full ${isLight ? 'bg-slate-100' : 'bg-[#1e293b]'} rounded-full overflow-hidden shadow-inner transition-colors duration-300`}>
                                <div 
                                    className={`h-full transition-all duration-1000 ease-out bg-gradient-to-r ${getGradeBarColor(avg, max, isLight)}`}
        ...
        {schoolData.tests.sort((a,b) => a.date.localeCompare(b.date)).map(test => {
            ...
            return (
                <div key={test.id} className={`flex items-center gap-6 p-6 ${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border ${isPast ? (isLight ? 'border-red-200 bg-red-50/30' : 'border-red-900/30') : ''} rounded-3xl transition-all duration-300 hover:scale-[1.01]`}>
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => startEditingTest(test)} className={`p-3 ${isLight ? 'bg-slate-100 text-slate-500 hover:text-blue-500 hover:bg-blue-50' : 'bg-slate-800/50 text-slate-500 hover:text-blue-400 hover:bg-blue-900/20'} rounded-2xl transition-all duration-300`}>
                        ...
                        <button onClick={() => deleteTest(test.id)} className={`p-3 ${isLight ? 'bg-slate-100 text-slate-500 hover:text-red-500 hover:bg-red-50' : 'bg-slate-800/50 text-slate-500 hover:text-red-400 hover:bg-red-900/20'} rounded-2xl transition-all duration-300`}>
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        </button>
                    </div>
                </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <div className={`flex items-center gap-4 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border p-4 rounded-3xl shadow-xl transition-all duration-300`}>
                        <div className="flex flex-col">
                            ...
                            <span className={`text-xs font-bold ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>Massimo:</span>
                        </div>
        ...
        <div className={`flex min-h-screen ${isLight ? 'bg-slate-50 text-slate-900' : 'bg-[#0b1220] text-slate-300'} font-sans selection:bg-blue-500/30 transition-colors duration-300`}>
            ...
            <div className={`w-16 md:w-20 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border-r flex flex-col items-center py-8 gap-8 flex-shrink-0 transition-colors duration-300`}>
                ...
                <button 
                    ...
                    className={`p-3 rounded-xl transition-all duration-200 ${activeTab === 'new_view' ? (isLight ? 'bg-blue-50 text-blue-600' : 'bg-blue-600/20 text-blue-400 shadow-inner') : (isLight ? 'text-slate-500 hover:text-slate-600 hover:bg-slate-100' : 'text-slate-500 hover:text-slate-300 hover:bg-[#1e293b]')}`}
                    title="Nuova Vista"
                >
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
