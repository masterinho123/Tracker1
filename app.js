...
const SchoolGradesView = ({ schoolData, setSchoolData, isLight }) => {
    ...
    const getGradeColor = (value, max = 10) => {
        ...
        if (isNaN(val) || val === 0) return 'text-slate-500 bg-slate-800 border-slate-700';
        
        // Se il voto è il massimo (o superiore per sicurezza), usiamo il verde smeraldo
        if (val >= m) return 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20 shadow-emerald-900/10';
    ...
    const getGradeBarColor = (value, max = 10) => {
        const val = parseFloat(value);
        const m = parseFloat(max) || 10;
        if (isNaN(val) || val === 0) return 'from-slate-600 to-slate-400';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        ...
                        <div className={`text-4xl font-black ${getGradeColor(totalAverageDisplay, schoolData.maxGrade || 10).split(' ')[0]} transition-colors duration-300`}>
                            {totalAverageDisplay}
                        </div>
                            ...
                            return (
                                <div key={test.id} className={`flex items-center gap-4 p-4 ${isLight ? 'bg-slate-50 border-slate-100' : 'bg-[#1e293b]/50 border-[#1f2937]'} rounded-2xl border transition-colors duration-300`}>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className={`text-xs ${isLight ? 'text-slate-500' : 'text-slate-500'}`}>{test.type} {test.note && `• ${test.note}`}</div>
                                    </div>
                                </div>
                            );
                        })}
                            ...
                            return (
                                <div key={subject.id} className="space-y-2">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className={`font-bold ${getGradeColor(avg, schoolData.maxGrade || 10).split(' ')[0]}`}>{avg > 0 ? avg : '-'}</span>
                                    </div>
                                ...
                                {subject.grades.map(grade => (
                                    <div key={grade.id} className="relative group/grade">
                                        <button 
                                            ...
                                            className={`w-12 h-12 rounded-2xl flex flex-col items-center justify-center font-black text-sm border-2 transition-all hover:scale-110 shadow-lg ${getGradeColor(grade.value, max)}`}
                                        >
                                    ...
                                    <div className="mt-1 space-y-1 overflow-y-auto max-h-[calc(100%-1.5rem)]">
                                        ...
                                        {dayGrades.map(g => (
                                            <button 
                                                ...
                                                className={`w-full text-left text-[9px] font-bold ${getGradeColor(g.value, schoolData.maxGrade || 10).split(' ')[1]} ${getGradeColor(g.value, schoolData.maxGrade || 10).split(' ')[0]} p-1 rounded-md truncate shadow-sm transition-all duration-300 hover:scale-105 active:scale-95`}
                                            >
    ...
    return (
        <div className="max-w-5xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <div className={`flex items-center gap-4 bg-[#0f172a] border border-[#1f2937] p-4 rounded-3xl shadow-xl ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} transition-all duration-300`}>
                        ...
                        <div className={`flex items-center gap-2 ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#020617] border-[#1f2937]'} px-3 py-2 rounded-2xl border transition-all duration-300`}>
                            <button 
                                ...
                                className={`w-8 h-8 flex items-center justify-center rounded-xl ${isLight ? 'bg-slate-200 hover:bg-slate-300 text-slate-600' : 'bg-slate-800 hover:bg-slate-700 text-slate-300'} transition-colors`}
                            >
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-3 pt-4">
                                <button 
                                    ...
                                    className={`flex-1 px-4 py-3 rounded-2xl font-bold ${isLight ? 'text-slate-400 hover:bg-slate-50' : 'text-slate-400 hover:bg-slate-800'} transition-all`}
                                >
                                    Annulla
                                </button>
                                <button 
