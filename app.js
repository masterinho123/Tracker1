    ...
    const renderTests = () => (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
                <h3 className={`text-xl font-bold ${isLight ? 'text-slate-700' : 'text-slate-100'} transition-colors`}>Registro Verifiche</h3>
                <button 
            ...
            <div className="space-y-4">
                {schoolData.tests.sort((a,b) => a.date.localeCompare(b.date)).map(test => {
                    ...
                    return (
                        <div key={test.id} className={`flex items-center gap-6 p-6 ${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border ${isPast ? (isLight ? 'border-red-200 bg-red-50/30' : 'border-red-900/30') : ''} rounded-3xl transition-all hover:scale-[1.01]`}>
                            ...
                            <div className="flex gap-2">
                                {isPast && (
                                    <div className={`flex items-center gap-2 ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#1e293b] border-[#1f2937]'} p-1 rounded-2xl border transition-colors`}>
                                        <input 
                ...
                {schoolData.tests.length === 0 && (
                    <div className={`text-center py-20 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border border-dashed rounded-3xl transition-colors`}>
        ...
        return (
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <h3 className={`text-2xl font-black ${isLight ? 'text-slate-700' : 'text-slate-100'} capitalize transition-colors`}>
                            {calendarDate.toLocaleString('it-IT', { month: 'long', year: 'numeric' })}
                        </h3>
                        <div className={`flex ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border rounded-xl overflow-hidden transition-colors`}>
                    ...
                    <div className="flex gap-4 text-xs font-bold uppercase tracking-widest">
                        <div className={`flex items-center gap-2 ${isLight ? 'text-slate-500' : 'text-slate-400'}`}><span className="w-3 h-3 bg-purple-500 rounded-full"></span> Verifica</div>
                        <div className={`flex items-center gap-2 ${isLight ? 'text-slate-500' : 'text-slate-400'}`}><span className="w-3 h-3 bg-green-500 rounded-full"></span> Voto</div>
                    </div>
                </div>

                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-2xl'} border rounded-3xl overflow-hidden transition-colors`}>
                    <div className={`grid grid-cols-7 border-b ${isLight ? 'border-slate-200 bg-slate-50' : 'border-[#1f2937] bg-[#1e293b]/50'} transition-colors`}>
                        {['Lun', 'Mar', 'Mer', 'Gio', 'Ven', 'Sab', 'Dom'].map(d => (
                            <div key={d} className={`py-4 text-center text-[10px] font-black ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest transition-colors`}>{d}</div>
                        ))}
                    </div>
                    <div className="grid grid-cols-7">
                        {days.map((date, i) => {
                            if (!date) return <div key={`empty-${i}`} className={`h-24 md:h-32 border-b border-r ${isLight ? 'border-slate-100 bg-slate-50/30' : 'border-[#1f2937] bg-[#0b1220]/30'} transition-colors`}></div>;
                            ...
                            return (
                                <div key={dateKey} className={`h-24 md:h-32 border-b border-r ${isLight ? 'border-slate-100' : 'border-[#1f2937]'} p-2 relative transition-colors hover:bg-blue-500/5 ${isToday ? (isLight ? 'bg-blue-50' : 'bg-blue-600/5') : ''}`}>
                                    <span className={`text-sm font-black ${isToday ? 'bg-blue-600 text-white w-6 h-6 flex items-center justify-center rounded-full shadow-lg shadow-blue-900/40' : isLight ? 'text-slate-700' : 'text-slate-500'} transition-colors`}>
                                        {date.getDate()}
                                    </span>
