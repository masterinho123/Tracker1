...
const Header = ({ totalHabits, completedHabits, totalPossible, currentDate, onNext, onPrev, onNextYear, onPrevYear, onExport, onImportClick, syncCode, setSyncCode, syncWord, setSyncWord, syncStatus, syncError, apiBase, setApiBase, supabaseConfig, setSupabaseConfig, isLight }) => {
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    >
                        ...
                        />
                        <div className={`w-[1px] h-3 ${isLight ? 'bg-slate-200' : 'bg-slate-700'} mx-1`}></div>
                        <input 
                            type="password" 
                            placeholder="Parola" 
                            value={syncWord}
                            ...
                            onClick={(e) => e.stopPropagation()}
                            className={`bg-transparent border-none outline-none text-[10px] ${isLight ? 'text-slate-600 placeholder:text-slate-400' : 'text-slate-300'} w-16 font-mono`}
                        />
                ...
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>Min</span>
                            <input 
                                type="number" 
                                value={schoolData.minGrade || 0}
                                onChange={(e) => updateGlobalMinGrade(e.target.value)}
                                className={`w-8 h-8 rounded-lg border ${isLight ? 'bg-white border-slate-200 text-red-600' : 'bg-[#1e293b] border-slate-700 text-red-400'} text-sm font-black text-center focus:outline-none focus:ring-2 focus:ring-red-500/20 transition-all`}
                            />
                        </div>
                        <div className="flex items-center gap-2">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>Max</span>
                            <input 
                                type="number" 
                                value={schoolData.maxGrade || 10}
                                onChange={(e) => updateGlobalMaxGrade(e.target.value)}
                                className={`w-8 h-8 rounded-lg border ${isLight ? 'bg-white border-slate-200 text-blue-600' : 'bg-[#1e293b] border-slate-700 text-blue-400'} text-sm font-black text-center focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all`}
                            />
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border ${isLight ? 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50' : 'bg-[#1e293b] border-slate-700 text-slate-300 hover:bg-slate-800'} transition-all duration-200 shadow-sm group`}
                    >
                        <svg className={`w-4 h-4 ${isLight ? 'text-blue-500' : 'text-blue-400'} group-hover:rotate-180 transition-transform duration-500`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
