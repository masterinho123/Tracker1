            ...
            <div className={`flex items-center justify-between flex-wrap gap-4 pt-4 border-t ${isLight ? 'border-slate-100' : 'border-[#1f2937]'}`}>
                ...
                <div className="flex-1 max-w-xs ml-auto">
                    <div className="flex justify-between mb-1 items-center">
                        <span className={`text-[10px] ${isLight ? 'text-slate-500' : 'text-slate-400'} uppercase font-bold tracking-wider`}>Completamento Mese</span>
                        <span className={`text-xs font-black ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>{progress.toFixed(1)}%</span>
                    </div>
                    <ProgressBar value={completedHabits} max={totalPossible} height="h-2.5" isLight={isLight} />
                </div>
            </div>
        </div>
    );
        ...
        <div className={`flex min-h-screen ${isLight ? 'bg-slate-50 text-slate-900' : 'bg-[#0b1220] text-slate-300'} font-sans selection:bg-blue-500/30 transition-colors duration-300`}>
            ...
            <div className={`w-16 md:w-20 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border-r flex flex-col items-center py-8 gap-8 flex-shrink-0 transition-colors duration-300`}>
                ...
                <div className="mt-auto flex flex-col gap-4 items-center">
                    <div 
                        onClick={openSyncSettings}
                        className={`flex items-center ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#0b1220] border-[#1f2937]'} border rounded-full px-2 py-1 cursor-pointer group relative transition-all duration-200 hover:scale-105 shadow-sm`}
                        title={`Backend: ${apiBase || 'locale'}${syncError ? '\nErrore: ' + syncError : ''}\nPC acceso: ${syncStatus === 'synced' ? 'Sì' : 'No'}\nClicca per configurare`}
                    >
                        <div className={`w-1.5 h-1.5 rounded-full mr-1.5 ${syncStatus === 'synced' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : syncStatus === 'syncing' ? 'bg-blue-500 animate-pulse' : 'bg-red-500'}`}></div>
                        <div className={`text-[9px] ${isLight ? 'text-slate-600' : 'text-slate-300'} font-mono mr-1.5 max-w-[45px] truncate`}>
                            {syncCode || 'Code'}
                        </div>
                        <span className={`text-[8px] ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-tighter group-hover:text-blue-400`}>Sync</span>
                    </div>
