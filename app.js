...
const SchoolGradesView = ({ schoolData, setSchoolData, isLight }) => {
    ...
    const getGradeColor = (value, max = 10) => {
        ...
        if (val >= max) return 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20 shadow-emerald-900/10';
    ...
    const getGradeBarColor = (value, max = 10) => {
        const val = parseFloat(value);
        if (val >= max) return 'from-emerald-600 to-emerald-400';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        ...
                        <div className={`text-4xl font-black ${getGradeColor(totalAverageDisplay).split(' ')[0]} transition-colors duration-300`}>
                            {totalAverageDisplay}
                        </div>
    ...
    return (
        <div className="max-w-5xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 bg-[#0f172a] border border-[#1f2937] p-1.5 rounded-2xl shadow-xl transition-all duration-300">
                        <button 
                            onClick={() => toggleTheme()}
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${!isLight ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/30' : 'text-slate-500 hover:text-slate-300'}`}
                        >
                        ...
                        <button 
                            onClick={() => toggleTheme()}
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${isLight ? 'bg-white text-slate-700 shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                        >
