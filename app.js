...
const useTableResizing = (initialLeft = 256, initialRight = 192) => {
    ...
    const startResizing = (e) => {
        ...
        const onMove = (ev) => {
            ...
            setLeftWidth(Math.min(500, Math.max(120, startW + dx)));
        };
        const onUp = () => {
    ...
    return { leftWidth, rightWidth, startResizing, startResizingRight };
};
...
const SimpleAreaChart = ({ values = [], color = "#10B981", maxY = 100, labels = [], datasets = [] }) => {
    const [hoverIndex, setHoverIndex] = useState(null);
    const containerRef = useRef(null);
    const activeDatasets = datasets.length > 0 ? datasets : [{ values, color, id: 'def' }];
    
    // Use fixed large width for SVG coordinate system to avoid jagged scaling
    const w = 1000;
    const h = 100;
    const firstValues = activeDatasets[0].values;
    ...
    return (
        <div ref={containerRef} className="w-full h-full relative"
            onMouseMove={(e) => {
                ...
                const idx = Math.round((relX / rect.width) * (firstValues.length - 1));
                setHoverIndex(clamp(idx, 0, firstValues.length - 1));
            }}
        ...
        >
            <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" width="100%" height="100%" style={{ display: 'block' }}>
                <defs>
                    {activeDatasets.map((ds, i) => (
                        <React.Fragment key={`defs-${ds.id || i}`}>
                            <linearGradient id={`grad-${ds.id || i}`} x1="0" y1="0" x2="0" y2="1">
                ...
                {activeDatasets.map((ds, i) => {
                    ...
                    return (
                        <path key={`fill-${ds.id || i}`} d={areaPath} fill={`url(#grad-${ds.id || i})`} />
                    );
                })}
                {/* Lines second so they are always on top */}
                 {activeDatasets.map((ds, i) => {
                     const pts = getChartPoints(ds.values, w, h, maxY);
                     const topPath = getSmoothPath(pts, 0.85);
                     return (
                         <React.Fragment key={`line-group-${ds.id || i}`}>
                             <path d={topPath} fill="none" stroke={ds.color} strokeWidth="3" strokeLinecap="round" style={{ vectorEffect: 'non-scaling-stroke' }} />
                             {pts.map((p, pi) => (
                                 <circle key={pi} cx={p[0]} cy={p[1]} r="2.5" fill={ds.color} stroke="#0f172a" strokeWidth="1" />
                             ))}
                         </React.Fragment>
                     );
                 })}
            ...
            {hoverIndex !== null && (
                <div style={{ left: `${(hoverIndex/(firstValues.length-1))*100}%` }} className="absolute top-0 bottom-0 w-px bg-slate-500/30 pointer-events-none">
                    <div className="absolute top-0 -translate-x-1/2 -translate-y-full bg-slate-800 text-slate-200 text-[10px] px-2 py-1 rounded shadow border border-[#1f2937] whitespace-nowrap z-50">
                        <div className="font-bold border-b border-slate-700 mb-1">{label(hoverIndex)}</div>
                        {activeDatasets.map((ds, i) => (
                            <div key={i} className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: ds.color }}></div>
                                <span>{ds.label || 'Valore'}: {Math.round(ds.values[hoverIndex] * 10) / 10}</span>
                            </div>
                        ))}
                    </div>
                </div>
                            ...
                            {ResponsiveContainer && AreaChart && Area ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData} margin={{ top: 20, right: 15, left: 5, bottom: 10 }}>
                                        ...
                                        <Area type="monotone" dataKey="mood" stroke="#ec4899" strokeWidth={3} fill="url(#colorMood)" animationDuration={1000} dot={{ r: 3, fill: '#ec4899', strokeWidth: 2, stroke: '#0f172a' }} activeDot={{ r: 5, strokeWidth: 0 }} />
                                        <Area type="monotone" dataKey="motivation" stroke="#3b82f6" strokeWidth={3} fill="url(#colorMot)" animationDuration={1000} dot={{ r: 3, fill: '#3b82f6', strokeWidth: 2, stroke: '#0f172a' }} activeDot={{ r: 5, strokeWidth: 0 }} />
                                        {Tooltip && <Tooltip 
                                            contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #1f2937', borderRadius: '8px', color: '#f1f5f9' }}
