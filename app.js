            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-2xl md:rounded-3xl p-4 md:p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        ...
                        <div className={`text-2xl md:text-4xl font-black ${getGradeColor(totalAverageDisplay, schoolData.maxGrade || 10, isLight).split(' ')[0]} transition-colors duration-300`}>
                            {totalAverageDisplay}
                        </div>
                        <div className={`mt-2 md:mt-4 flex items-center gap-1 md:gap-2 text-[10px] md:text-sm ${isLight ? 'text-slate-500' : 'text-slate-400'} transition-colors duration-300`}>
                            <span className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${parseFloat(totalAverageDisplay) >= ((schoolData.maxGrade || 10) * 0.6) ? 'bg-green-500' : 'bg-red-500'}`}></span>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-2xl md:rounded-3xl p-4 md:p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-[10px] md:text-xs font-bold ${isLight ? 'text-slate-500' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Top</div>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-2xl md:rounded-3xl p-4 md:p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-[10px] md:text-xs font-bold ${isLight ? 'text-slate-500' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Worst</div>
                        ...
                        <div key={subject.id} className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-sm'} border rounded-2xl md:rounded-3xl p-4 md:p-6 hover:shadow-xl transition-all group relative overflow-hidden duration-300`}>
                            ...
                            <div className="flex items-center justify-between mb-4 md:mb-6">
                                ...
                                <div className="flex items-center gap-2 md:gap-4">
                                    <div className={`text-base md:text-xl font-black px-3 md:px-4 py-1.5 md:py-2 rounded-xl md:rounded-2xl ${getGradeColor(avg, max, isLight)}`}>
                                        {avg > 0 ? avg : '-'}
                                    </div>
                                    <button 
                                ...
                                {subject.grades.map(grade => (
                                    <div key={grade.id} className="relative group/grade">
                                        <button 
                                            ...
                                            className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex flex-col items-center justify-center font-black text-xs md:text-sm border-2 transition-all hover:scale-110 shadow-lg ${getGradeColor(grade.value, max, isLight)}`}
                                        >
