            ...
            {weeks.map((v, i) => {
                ...
                return (
                    <div key={i} className="flex flex-col items-center">
                        ...
                        <div className={`text-[11px] ${isLight ? 'text-slate-500' : 'text-slate-300'} mt-1`}>S{i+1}</div>
                    </div>
                );
            })}
        </div>
        ...
        <div className={`${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border rounded-xl shadow-sm overflow-hidden flex flex-col transition-colors duration-300`}>
            <div className="flex">
                        ...
                        <div className="min-w-max">
                            ...
                            <div className={`border-t ${isLight ? 'border-slate-100' : 'border-[#1f2937]'} transition-colors duration-300`}>
                                ...
                                <div className={`flex h-8 ${isLight ? 'bg-white border-slate-100' : 'bg-[#0f172a] border-[#1f2937]'} border-b transition-colors duration-300`}>
                                    {dailyStats.map(stat => (
                                        <div key={stat.day} className={`flex-shrink-0 flex items-center justify-center border-r ${isLight ? 'border-slate-100' : 'border-[#1f2937] text-slate-300'} text-[10px]`} style={{ width: cellWidth }}>
                                            {stat.done}
                                        </div>
                                    ))}
                                </div>
                ...
                <div className={`flex-shrink-0 border-l ${isLight ? 'border-slate-100 bg-white' : 'border-[#1f2937] bg-[#0f172a]'} transition-colors duration-300`} style={{ width: rightWidth }}>
                    ...
                    <div className={`flex h-6 ${isLight ? 'bg-slate-50 border-slate-100 text-slate-400' : 'bg-[#0b1220] border-[#1f2937] text-slate-400'} border-b text-[10px] transition-colors duration-300`}>
                     ...
                     {habits.map(habit => {
                        ...
                        return (
                            <div key={habit.id} className={`h-10 border-b ${isLight ? 'border-slate-100' : 'border-[#1f2937]'} flex items-center px-2 transition-colors duration-300`}>
                                ...
                                <div className={`w-10 text-xs text-center font-bold ${isLight ? 'text-slate-700' : 'text-slate-200'} cursor-help transition-colors duration-300`} title={`${percentage}%`}>{actual}</div>
...
const TodayInfo = ({ isLight }) => {
    ...
    return (
        <div className={`${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border p-3 rounded-xl shadow-sm mb-4 flex items-center transition-colors duration-300`}>
            <div className={`px-3 py-1 rounded-full ${isLight ? 'bg-slate-100 text-slate-600' : 'bg-slate-800 text-slate-200'} text-sm font-medium transition-colors duration-300`}>
             ...
             <div className="flex">
                ...
                <div className={`flex-shrink-0 border-r ${isLight ? 'border-slate-200 bg-white' : 'border-[#1f2937] bg-[#0f172a]'} z-10 transition-colors duration-300`} style={{ width: leftWidth }}>
                    ...
                    <div className={`h-20 border-t ${isLight ? 'border-slate-200 bg-white' : 'border-[#1f2937] bg-[#0f172a]'} flex items-center justify-center gap-4 transition-colors duration-300`}>
                        <div className={`px-3 py-1 rounded-full ${isLight ? 'bg-pink-100 text-pink-600' : 'bg-pink-900/30 text-pink-300'} text-[10px] font-bold transition-colors duration-300`}>
                            Media Umore: {moodAvg.toFixed(1)}
                        </div>
                        <div className={`px-3 py-1 rounded-full ${isLight ? 'bg-blue-100 text-blue-600' : 'bg-blue-900/30 text-blue-300'} text-[10px] font-bold transition-colors duration-300`}>
...
const SchoolGradesView = ({ schoolData, setSchoolData, isLight }) => {
    ...
    const getGradeColor = (value, max = 10, isLight = false) => {
        const val = parseFloat(value);
        ...
        // Se il voto è il massimo (o superiore per sicurezza), usiamo il verde smeraldo
        ...
        const percent = (val / m) * 10;
        
        // I colori ora scalano proporzionalmente al massimo impostato
        // percent è normalizzata su base 10 per riutilizzare le classi esistenti
        if (percent < 5) return isLight ? 'text-red-600 bg-red-100 border-red-200' : 'text-red-500 bg-red-500/10 border-red-500/20 shadow-red-900/10';
        if (percent < 6) return isLight ? 'text-orange-600 bg-orange-100 border-orange-200' : 'text-orange-500 bg-orange-500/10 border-orange-500/20 shadow-orange-900/10';
        if (percent < 7) return isLight ? 'text-yellow-600 bg-yellow-100 border-yellow-200' : 'text-yellow-500 bg-yellow-500/10 border-yellow-500/20 shadow-yellow-900/10';
        if (percent < 8) return isLight ? 'text-green-600 bg-green-100 border-green-200' : 'text-green-400 bg-green-400/10 border-green-400/20 shadow-green-900/10';
        if (percent < 9) return isLight ? 'text-cyan-600 bg-cyan-100 border-cyan-200' : 'text-cyan-400 bg-cyan-400/10 border-cyan-400/20 shadow-cyan-900/10';
        return isLight ? 'text-blue-600 bg-blue-100 border-blue-200' : 'text-blue-400 bg-blue-400/10 border-blue-400/20 shadow-blue-900/10';
    };

    const getGradeBarColor = (value, max = 10, isLight = false) => {
        const val = parseFloat(value);
        const m = parseFloat(max) || 10;
        ...
        if (percent < 5) return isLight ? 'from-red-600 to-red-400' : 'from-red-600 to-red-400';
        if (percent < 6) return isLight ? 'from-orange-600 to-orange-400' : 'from-orange-600 to-orange-400';
        if (percent < 7) return isLight ? 'from-yellow-600 to-yellow-400' : 'from-yellow-600 to-yellow-400';
        if (percent < 8) return isLight ? 'from-green-600 to-green-400' : 'from-green-600 to-green-400';
        if (percent < 9) return isLight ? 'from-cyan-600 to-cyan-400' : 'from-cyan-600 to-cyan-400';
        return isLight ? 'from-blue-600 to-blue-400' : 'from-blue-600 to-blue-400';
    };
    ...
    const renderDashboard = () => (
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-xs font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Media Generale</div>
                        <div className={`text-4xl font-black ${getGradeColor(totalAverageDisplay, schoolData.maxGrade || 10, isLight).split(' ')[0]} transition-colors duration-300`}>
                            {totalAverageDisplay}
                        </div>
                        <div className={`mt-4 flex items-center gap-2 text-sm ${isLight ? 'text-slate-500' : 'text-slate-400'} transition-colors duration-300`}>
                            <span className={`w-2 h-2 rounded-full ${parseFloat(totalAverageDisplay) >= ((schoolData.maxGrade || 10) * 0.6) ? 'bg-green-500' : 'bg-red-500'}`}></span>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-xs font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Prossime Verifiche</div>
                        <div className={`text-4xl font-black ${isLight ? 'text-purple-600' : 'text-purple-400'}`}>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-xs font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Migliore</div>
                        <div className={`text-2xl font-black ${isLight ? 'text-green-600' : 'text-green-400'} truncate`}>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border rounded-3xl p-6 relative overflow-hidden group transition-all duration-300`}>
                    ...
                    <div className="relative">
                        <div className={`text-xs font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest mb-1 transition-colors duration-300`}>Da Migliorare</div>
                        <div className={`text-2xl font-black ${isLight ? 'text-red-600' : 'text-red-400'} truncate`}>
                            ...
                            return (
                                <div key={subject.id} className="space-y-2">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className={`font-bold ${getGradeColor(avg, schoolData.maxGrade || 10, isLight).split(' ')[0]}`}>{avg > 0 ? avg : '-'}</span>
                                    </div>
                                    <div className={`h-1.5 w-full ${isLight ? 'bg-slate-100' : 'bg-[#1e293b]'} rounded-full overflow-hidden transition-colors duration-300`}>
                                        <div 
                                            className={`h-full transition-all duration-500 bg-gradient-to-r ${getGradeBarColor(avg, schoolData.maxGrade || 10, isLight)}`}
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {schoolData.subjects.map(subject => {
                    ...
                    const max = schoolData.maxGrade || 10;
                    return (
                        <div key={subject.id} className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-sm'} border rounded-3xl p-6 hover:shadow-xl transition-all group relative overflow-hidden duration-300`}>
                            <div className="absolute top-0 left-0 w-1 h-full bg-blue-600 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex-1">
                                    <h3 className={`text-2xl font-black ${isLight ? 'text-slate-700' : 'text-slate-100'} group-hover:text-blue-400 transition-colors duration-300`}>{subject.name}</h3>
                                    <div className="text-xs font-bold text-slate-500 mt-1 uppercase tracking-widest">{subject.grades.length} voti</div>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className={`text-xl font-black px-4 py-2 rounded-2xl ${getGradeColor(avg, max, isLight)}`}>
                                        {avg > 0 ? avg : '-'}
                                    </div>
                                    <button 
                            ...
                            <div className="flex flex-wrap gap-3 mb-6 min-h-[48px]">
                                {subject.grades.map(grade => (
                                    <div key={grade.id} className="relative group/grade">
                                        <button 
                                            ...
                                            className={`w-12 h-12 rounded-2xl flex flex-col items-center justify-center font-black text-sm border-2 transition-all hover:scale-110 shadow-lg ${getGradeColor(grade.value, max, isLight)}`}
                                        >
                                ...
                                {addingGradeTo === subject.id ? null : (
                                    <button 
                                        ...
                                        className={`w-12 h-12 rounded-2xl border-2 border-dashed ${isLight ? 'border-slate-200' : 'border-[#1f2937]'} flex items-center justify-center text-slate-600 hover:border-blue-500 hover:text-blue-500 transition-all cursor-pointer group/add`}
                                    >
                            ...
                            <div className={`h-2 w-full ${isLight ? 'bg-slate-100' : 'bg-[#1e293b]'} rounded-full overflow-hidden shadow-inner transition-colors duration-300`}>
                                <div 
    ...
    const renderTests = () => (
        <div className="space-y-6">
            ...
            {showAddTest && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className={`${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border rounded-3xl p-8 max-w-md w-full shadow-2xl transition-colors`}>
                        <h4 className={`text-2xl font-black ${isLight ? 'text-slate-700' : 'text-slate-100'} mb-6 transition-colors`}>Pianifica Verifica</h4>
                        <form onSubmit={addTest} className="space-y-4">
                                ...
                                <div className="flex gap-2">
                                    {['Scritto', 'Orale', 'Pratico'].map(t => (
                                        <button 
                                            ...
                                            className={`flex-1 py-2 rounded-xl text-sm font-bold transition-all ${testForm.type === t ? 'bg-purple-600 text-white' : isLight ? 'bg-slate-100 text-slate-400 hover:bg-slate-200' : 'bg-[#1e293b] text-slate-400 hover:bg-slate-800'}`}
                                        >
                                            {t}
                                        </button>
                                    ))}
                            ...
                            <div className="flex gap-3 pt-4">
                                <button type="button" onClick={() => setShowAddTest(false)} className={`flex-1 px-4 py-3 rounded-2xl font-bold ${isLight ? 'text-slate-400 hover:bg-slate-50' : 'text-slate-400 hover:bg-slate-800'} transition-all`}>Annulla</button>
        ...
        {schoolData.tests.sort((a,b) => a.date.localeCompare(b.date)).map(test => {
            ...
            return (
                <div key={test.id} className={`flex items-center gap-6 p-6 ${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border ${isPast ? (isLight ? 'border-red-200 bg-red-50/30' : 'border-red-900/30') : ''} rounded-3xl transition-all duration-300 hover:scale-[1.01]`}>
                    <div className={`flex flex-col items-center justify-center w-16 h-16 ${isPast ? (isLight ? 'bg-red-100 text-red-600' : 'bg-red-900/20 text-red-400') : 'bg-purple-600/20 text-purple-400'} rounded-2xl transition-colors duration-300`}>
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-2">
                            <h4 className={`text-xl font-black ${isLight ? 'text-slate-700' : 'text-slate-100'} transition-colors duration-300`}>{subject?.name || 'Materia eliminata'}</h4>
                            <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider ${test.type === 'Scritto' ? (isLight ? 'bg-blue-100 text-blue-600' : 'bg-blue-500/10 text-blue-400') : (isLight ? 'bg-orange-100 text-orange-600' : 'bg-orange-500/10 text-orange-400')} transition-colors duration-300`}>
                                {test.type}
                            </span>
                        </div>
                        <p className={`${isLight ? 'text-slate-500' : 'text-slate-500'} font-medium transition-colors duration-300`}>{test.note || 'Nessuna nota aggiunta'}</p>
                    </div>
                    <div className="flex gap-2">
                        {isPast && (
                            <div className={`flex items-center gap-2 ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#1e293b] border-[#1f2937]'} p-1 rounded-2xl border transition-colors duration-300`}>
                                <input 
                                    type="number" step="0.5" min="1" max={schoolData.maxGrade || 10} placeholder="Voto"
                                    className={`w-16 bg-transparent text-center font-black ${isLight ? 'text-blue-600' : 'text-blue-400'} focus:outline-none`}
                                    onKeyDown={e => {
                                        if (e.key === 'Enter') {
                                ...
                                />
                                <button onClick={(e) => completeTest(test, e.target.previousSibling.value)} className="bg-green-600 p-2 rounded-xl text-white hover:bg-green-500 transition-all duration-300">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                                </button>
                            </div>
                        )}
                        <button onClick={() => deleteTest(test.id)} className={`p-3 ${isLight ? 'bg-slate-100 text-slate-400 hover:text-red-500 hover:bg-red-50' : 'bg-slate-800/50 text-slate-500 hover:text-red-400 hover:bg-red-900/20'} rounded-2xl transition-all duration-300`}>
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        </button>
                    </div>
                </div>
                ...
                {schoolData.tests.length === 0 && (
                    <div className={`text-center py-20 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border border-dashed rounded-3xl transition-colors duration-300`}>
                        <p className={`font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>Nessuna verifica programmata nel registro.</p>
                    </div>
                )}
            </div>
        </div>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div className={`flex ${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937]'} border rounded-xl overflow-hidden transition-colors duration-300`}>
                            <button onClick={prevMonth} className={`p-2 ${isLight ? 'hover:bg-slate-50' : 'hover:bg-[#1e293b]'} text-slate-400 transition-colors duration-300`}>
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7" /></svg>
                            </button>
                            <button onClick={nextMonth} className={`p-2 ${isLight ? 'hover:bg-slate-50' : 'hover:bg-[#1e293b]'} text-slate-400 transition-colors duration-300 border-l ${isLight ? 'border-slate-200' : 'border-[#1f2937]'}`}>
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7" /></svg>
                            </button>
                            <button 
                                onClick={() => setCalendarDate(new Date())}
                                className={`px-3 py-1 text-[10px] font-black uppercase tracking-tighter ${isLight ? 'hover:bg-slate-50 text-slate-400' : 'hover:bg-[#1e293b] text-slate-500'} transition-colors duration-300 border-l ${isLight ? 'border-slate-200' : 'border-[#1f2937]'}`}
                            >
                                Oggi
                            </button>
                        </div>
                ...
                <div className={`${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-2xl'} border rounded-3xl overflow-hidden transition-colors duration-300`}>
                    <div className={`grid grid-cols-7 border-b ${isLight ? 'border-slate-200 bg-slate-50' : 'border-[#1f2937] bg-[#1e293b]/50'} transition-colors duration-300`}>
                        {['Lun', 'Mar', 'Mer', 'Gio', 'Ven', 'Sab', 'Dom'].map(d => (
                            <div key={d} className={`py-4 text-center text-[10px] font-black ${isLight ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest transition-colors duration-300`}>{d}</div>
                        ))}
                    </div>
                            ...
                            return (
                                <div key={dateKey} className={`h-24 md:h-32 border-b border-r ${isLight ? 'border-slate-100' : 'border-[#1f2937]'} p-2 relative transition-all duration-300 hover:bg-blue-500/5 ${isToday ? (isLight ? 'bg-blue-50' : 'bg-blue-600/5') : ''}`}>
                                    ...
                                    <div className="mt-1 space-y-1 overflow-y-auto max-h-[calc(100%-1.5rem)]">
                                        {dayTests.map(t => (
                                            <div key={t.id} className={`text-[9px] font-bold bg-purple-600 text-white p-1 rounded-md truncate shadow-sm transition-all duration-300`}>
                                        ...
                                        {dayGrades.map(g => (
                                            <button 
                                                ...
                                                className={`w-full text-left text-[9px] font-bold ${getGradeColor(g.value, schoolData.maxGrade || 10, isLight).split(' ')[1]} ${getGradeColor(g.value, schoolData.maxGrade || 10, isLight).split(' ')[0]} p-1 rounded-md truncate shadow-sm transition-all duration-300 hover:scale-105 active:scale-95`}
                                            >
    ...
    return (
        <div className="max-w-5xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <div className={`flex items-center gap-4 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border p-4 rounded-3xl shadow-xl transition-all duration-300`}>
                        <div className="flex flex-col">
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-tight">Scala Voti</span>
                            <span className={`text-xs font-bold ${isLight ? 'text-slate-400' : 'text-slate-400'}`}>Massimo:</span>
                        </div>
                        <div className={`flex items-center gap-2 ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#020617] border-[#1f2937]'} px-3 py-2 rounded-2xl border transition-all duration-300`}>
                            <button 
                                onClick={() => updateGlobalMaxGrade((schoolData.maxGrade || 10) - 1)}
                                className={`w-8 h-8 flex items-center justify-center rounded-xl ${isLight ? 'bg-slate-100 hover:bg-slate-200 text-slate-600' : 'bg-slate-800 hover:bg-slate-700 text-slate-300'} transition-colors`}
                            >
                            ...
                            <input 
                                ...
                                className={`w-10 bg-transparent text-xl font-black ${isLight ? 'text-blue-600' : 'text-blue-400'} text-center focus:outline-none`}
                            />
                            <button 
                                onClick={() => updateGlobalMaxGrade((schoolData.maxGrade || 10) + 1)}
                                className={`w-8 h-8 flex items-center justify-center rounded-xl ${isLight ? 'bg-slate-100 hover:bg-slate-200 text-slate-600' : 'bg-slate-800 hover:bg-slate-700 text-slate-300'} transition-colors`}
            ...
            <div className={`flex p-1.5 ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border rounded-3xl mb-10 w-fit shadow-inner transition-all duration-300`}>
                {[
                    ...
                    <button 
                        ...
                        className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-bold transition-all duration-300 ${activeSchoolTab === tab.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/30' : isLight ? 'text-slate-500 hover:text-slate-800 hover:bg-slate-100' : 'text-slate-500 hover:text-slate-300 hover:bg-[#1e293b]'}`}
                    >
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-3 pt-4">
                                <button 
                                    ...
                                    className={`flex-1 px-4 py-3 rounded-2xl font-bold ${isLight ? 'text-slate-400 hover:bg-slate-100' : 'text-slate-400 hover:bg-slate-800'} transition-all`}
                                >
                                    Annulla
                                </button>
                                <button 
