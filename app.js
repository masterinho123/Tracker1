                            ...
                            <div className="flex flex-wrap gap-3 mb-6 min-h-[48px]">
                                ...
                                {addingGradeTo === subject.id ? (
                                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                                        <div className={`${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border rounded-3xl p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200 transition-colors`}>
                                            <div className="flex justify-between items-center mb-6">
                                                <h4 className={`text-xl font-black ${isLight ? 'text-slate-700' : 'text-slate-100'} transition-colors`}>Nuovo Voto</h4>
                                            ...
                                            <div className="space-y-4">
                                                <div className="space-y-1">
                                                    ...
                                                    <input 
                                                        ...
                                                        className={`w-full ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-[#1e293b] border-[#1f2937]'} border rounded-2xl px-4 py-3 text-2xl font-black text-blue-400 focus:outline-none focus:border-blue-500 transition-all`}
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                {schoolData.tests.sort((a,b) => a.date.localeCompare(b.date)).map(test => {
                    ...
                    return (
                        <div key={test.id} className={`flex items-center gap-6 p-6 ${isLight ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0f172a] border-[#1f2937] shadow-xl'} border ${isPast ? (isLight ? 'border-red-200 bg-red-50/30' : 'border-red-900/30') : ''} rounded-3xl transition-all hover:scale-[1.01]`}>
                            <div className={`flex flex-col items-center justify-center w-16 h-16 ${isPast ? (isLight ? 'bg-red-100 text-red-600' : 'bg-red-900/20 text-red-400') : 'bg-purple-600/20 text-purple-400'} rounded-2xl transition-colors`}>
                            ...
                            <div className="flex-1">
                                <div className="flex items-center gap-2">
                                    <h4 className={`text-xl font-black ${isLight ? 'text-slate-700' : 'text-slate-100'} transition-colors`}>{subject?.name || 'Materia eliminata'}</h4>
                                ...
                                <p className={`${isLight ? 'text-slate-500' : 'text-slate-500'} font-medium transition-colors`}>{test.note || 'Nessuna nota aggiunta'}</p>
                            </div>
                            <div className="flex gap-2">
                                {isPast && (
                                ...
                                <button onClick={() => deleteTest(test.id)} className={`p-3 ${isLight ? 'bg-slate-100 text-slate-400 hover:text-red-500' : 'bg-slate-800/50 text-slate-500 hover:text-red-400'} rounded-2xl transition-all`}>
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                </button>
                            </div>
                        </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div className={`flex ${isLight ? 'bg-white border-slate-200' : 'bg-[#0f172a] border-[#1f2937]'} border rounded-xl overflow-hidden transition-colors duration-300`}>
                            <button onClick={prevMonth} className={`p-2 ${isLight ? 'hover:bg-slate-50' : 'hover:bg-[#1e293b]'} text-slate-400 transition-colors`}>
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7" /></svg>
                            </button>
                            <button onClick={nextMonth} className={`p-2 ${isLight ? 'hover:bg-slate-50' : 'hover:bg-[#1e293b]'} text-slate-400 transition-colors border-l ${isLight ? 'border-slate-200' : 'border-[#1f2937]'}`}>
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7" /></svg>
                            </button>
                            <button 
                                onClick={() => setCalendarDate(new Date())}
                                className={`px-3 py-1 text-[10px] font-black uppercase tracking-tighter ${isLight ? 'hover:bg-slate-50 text-slate-400' : 'hover:bg-[#1e293b] text-slate-500'} transition-colors border-l ${isLight ? 'border-slate-200' : 'border-[#1f2937]'}`}
                            >
                                Oggi
                            </button>
                        </div>
                    ...
                    <div className="flex gap-4 text-xs font-bold uppercase tracking-widest">
                        <div className={`flex items-center gap-2 ${isLight ? 'text-slate-500' : 'text-slate-400'} transition-colors duration-300`}><span className="w-3 h-3 bg-purple-500 rounded-full"></span> Verifica</div>
                        <div className={`flex items-center gap-2 ${isLight ? 'text-slate-500' : 'text-slate-400'} transition-colors duration-300`}><span className="w-3 h-3 bg-green-500 rounded-full"></span> Voto</div>
                    </div>
                </div>
